...
export function main() {
  ...
  let colorScaleList = {
      ...
      customFunc: function (name, arr, num) {
        ...
        const arrThresholds = d3
          .range(0, colorLength)
          .map((e) => e / (colorLength - 1));
        let colorTemperature = d3
          .scaleLinear()
      ...
      d3colorChosefunc: function (name, num) {
        ...
        if (d3[`scheme${name}`]) {
          if (typeof d3[`scheme${name}`][0] !== "string") {
            colors = (
              d3[`scheme${name}`][n] ||
              d3[`scheme${name}`][d3[`scheme${name}`].length - 1]
            ).slice();
          } else colors = d3[`scheme${name}`].slice();
        } else {
    ...
    colorArr = {
      ...
      Cluster: [
        { val: "Category10", type: "d3", label: "D3" },
        { val: "Paired", type: "d3", label: "Blue2Red" },
      ],
    };
