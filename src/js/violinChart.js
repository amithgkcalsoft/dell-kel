...
export function viiolinplot() {
  ...
  viiolinplot.draw = function (contain, allElementsObj) {
    // h
    let getpos_func = (p) => {
      return p.attr(
        "transform",
        (d, i) =>
          `translate(${graphicopt.margin.left},${
            graphicopt.margin.top + gpos(d.axis + i)
          })`
      );
    };
    let laxis = (p) => {
    ...
    let tickLower = (p) => {
      return p
        .styles({ "text-anchor": "end", "stroke-width": 0 })
        .text(d3.format(".2s")(ticksDisplay[0]))
        .attrs({
          dx: -1,
          dy: 4,
        });
    ...
    let tickHigher = (p) => {
      return p
        .styles({ "text-anchor": "start", "stroke-width": 0 })
        .text(d3.format(".2s")(ticksDisplay[1]))
        .attrs({
          dx: 1,
          x: h.range()[1],
          dy: 4,
    ...
    if (graphicopt.direction === "v") {
      getpos_func = (p) => {
        return p.attr(
          "transform",
          (d, i) =>
            `translate(${graphicopt.margin.left + gpos(d.axis + i)},${
              graphicopt.margin.top
            })`
        );
      };
      laxis = (p) => {
    ...
    let axisg = viol_n
      ...
      .attr("class", "gvisaxis")
      .style("stroke", "black");
    axisg.append("line").attr("class", "laxis").call(laxis);
    ...
    if (graphicopt.tick === undefined && graphicopt.tick.visibile != false) {
      ...
      axisg.append("line").attr("class", "tick1").call(tick1);
      axisg.append("line").attr("class", "tick2").call(tick2);
      if (ticksDisplay.length) {
        ...
        axisg.append("text").attr("class", "tickDisplay lower").call(tickLower);

        axisg
          .append("text")
          .attr("class", "tickDisplay higher")
          .call(tickHigher);
      }
    }
    ...
    viol_chart
      ...
      .attr(
        "d",
        (d) => createviolin(d.arr) // This makes the line smoother to give the violin appearance. Try d3.curveStep to see the difference
      );

    let median_rect = viol_chart
      .selectAll("rect.median")
      .data((d) => (d.median !== undefined ? [d.median] : []))
      .call(draw_median);
  ...
  function handledata(data) {
    ...
    let xNumrange = (graphicopt.single_w || gpos.step()) / 2;
    if (graphicopt.isStack)
      if (data.length === 1) xNumrange = xNumrange * 2;
      else xNumrange = graphicopt.widthG() / data.length;
    ...
    if (graphicopt.opt.dataformated) {
      ...
      kde = kernelDensityEstimator(
        ...
        h.ticks(graphicopt.opt.resolution)
      );
  ...
  viiolinplot.graphicopt = function (_) {
